...
export const transformSourcesChartData = (data: SourceTimeSeries[]) => {
  return data.reduce(
    (result: { series: any[]; categories: string[] }, obj) => {
      let series = result.series.find(
        (series) => series.name === obj.source_id
      );
      if (!series) {
...
export const transformDestinationsChartData = (
  data: DestinationTimeSeries[]
) => {
  return data.reduce(
    (result: { series: any[]; categories: string[] }, obj) => {
      let series = result.series.find(
        (series) => series.name === obj.destination_id
      );
      if (!series) {
