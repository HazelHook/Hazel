...
const uniques: Message[] = [
	...
	},
	{
		...
		data: {
			...
			data: {
				workspace_id: "usr_sdkl32d0pe2ss",
				level: "premium",
				plan: "yearly",
				state: "confirmed",
			},
...
function buildMessageBox(
	...
	const menuItems = ["Details", "Headers", "Body"]
	...
	if (messages.length === 0) {
		...
		const data = {
			"Receieved at": `${message.received_at.toDateString()} ${prettyTimestamp(
				message.received_at,
			)}:${message.received_at.getMilliseconds()}`,
		...
		const query = Object.keys(message.query)
			.map((key) => `${key.padEnd(longestQuery)}: ${message.query[key]}`)
			.join("\n")
		...
		const content = Object.keys(message.headers)
			.map((key) => `${key.padEnd(longest)}: ${message.headers[key]}`)
			.join("\n")
	...
	const controls = ["Dashboard", "Retry Request", "Copy cURL", "Copy Body"]
