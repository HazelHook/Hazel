...
export function getForwardMenu(data: UserData, module: Module, rerender: (box: blessed.Widgets.BoxElement) => void) {
	...
	indicators.setContent(data.destinations.map((d) => "{bold}{red-fg}●{/red-fg}{/bold}").join("\n"))
	...
	const content = renderList({
		...
		Sources: data.connections
			.filter((c) => c.destination === destination.id)
			.map((c) => data.sources.find((s) => s.id === c.source)?.name)
			.join(", "),
	})
