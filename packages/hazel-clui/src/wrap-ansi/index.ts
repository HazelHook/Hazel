...
const wrapAnsiHyperlink = (uri: string) => `${ESCAPES.values().next().value}${ANSI_ESCAPE_LINK}${uri}${ANSI_ESCAPE_BELL}`;
...
const exec = (string: string, columns: number, options: Options = {}) => {
	...
	for (const [index, character] of pre.entries()) {
		...
		if (ESCAPES.has(character)) {
			const regex = new RegExp(`(?:\\${ANSI_CSI}(?<code>\\d+)m|\\${ANSI_ESCAPE_LINK}(?<uri>.*)${ANSI_ESCAPE_BELL})`).exec(pre.slice(index).join(''));
