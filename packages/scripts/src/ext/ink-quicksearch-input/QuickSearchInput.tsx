import React, { Component, FC, FunctionComponent, PropsWithChildren, useEffect, useMemo, useRef, useState } from "react"
...
const HighlightComponent: FC = ({ children }) => <Color bgHex="#6C71C4">{children}</Color>
...
const StatusComponent: FC<StatusProps> = ({ hasMatch, children, label }) => (
	<Text>
		{`${label || "Query"}: `}
		<Color hex={"#74BEFF"}>{children}</Color>
	</Text>
)
...
export const QuickSearch: FC<QuickSearchProps> = (props) => {
	...
	function getMatchIndex(label: string, query: string) {
		return props.caseSensitive ? label.indexOf(query) : label.toLowerCase().indexOf(query.toLowerCase())
	}
