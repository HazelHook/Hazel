...
import { Box, Text, useStdin, type BoxProps, type StdinProps } from "../ink"
...
export const Tab: React.FunctionComponent<TabProps> = ({ children }) => <>{children}</>
...
class TabsWithStdin extends React.Component<TabsWithStdinProps, TabsWithStdinState> {
	...
	componentDidMount(): void {
		const { stdin, setRawMode, isRawModeSupported, children, defaultValue } = this.props
		...
		if (typeof defaultValue !== "undefined") {
			const foundIndex = children.findIndex((child) => child.props.name === defaultValue)
	...
	handleKeyPress(ch: string, key: null | { name: string; shift: boolean; meta: boolean }): void {
	...
	render(): React.ReactNode {
		const { children, flexDirection, width, isFocused, showIndex, colors: colorsProp, ...rest } = this.props
		const { activeTab } = this.state
...
export const Tabs: React.FunctionComponent<TabsProps> = (props) => {
	...
	return <TabsWithStdin isRawModeSupported={isRawModeSupported} stdin={stdin} setRawMode={setRawMode} {...props} />
}
