...
export const FormFieldRenderer: React.FC<FormFieldRendererProps<any>> = (
  props
) => {
  const manager = getManager(props.field.type, props.customManagers);
  const [error, setError] = useState<string>();
  const [currentValue, setCurrentValue] = useState<any>(
    props.value ?? props.field.initialValue
  );

  const isEditing =
    !!props.editingField &&
    props.editingField === (props.field.label ?? props.field.name);
  ...
  useInput(
    (input, key) => {
      if (!isEditing && key.return && !key.ctrl && !key.meta) {
        ...
        isEditing &&
        key.return &&
        (!manager?.needCtrlToReturnSave || key.ctrl)
      ) {
        save();
      }
    },
  ...
  if (!isEditing) {
    ...
    if (!manager) {
      component = (
        <Text color="red">
          No formfield manager for form field of type {props.field.type}{" "}
          available.
        </Text>
      );
    } else {
        ...
        <Box marginTop={2}>
          <Text dimColor>
            {error ? (
              ...
              <>
                Press {manager?.needCtrlToReturnSave ? "CTRL+Enter" : "Enter"}{" "}
