...
export class StringFormFieldManager
  implements FormFieldManager<FormFieldString>
{
      ...
      <TextInput
        ...
        onChange={(value) => {
          ...
          if (props.field.regex && !props.field.regex.test(value)) {
            props.onError(
              `Value does not conform to regular expression: ${props.field.regex}`
            );
          } else {
            props.onClearError();
          }
  ...
  public renderValue: React.FC<FormFieldValueRendererProps<FormFieldString>> = (
    props
  ) => (
    <Transform
      transform={
