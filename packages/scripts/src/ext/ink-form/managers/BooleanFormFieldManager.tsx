...
export class BooleanFormFieldManager
  implements FormFieldManager<FormFieldBoolean>
{
  public type: TypeOfField<FormFieldBoolean> = "boolean";

  public renderField: React.FC<
    SpecificFormFieldRendererProps<FormFieldBoolean>
  > = (props) => {
    useInput((input, key) => {
      if (input === " ") {
    ...
    return (
      <Box
        width="100%"
        borderStyle={"round"}
        borderColor={props.value ? "green" : "gray"}
        marginBottom={2}
      >
        <Box marginRight={2}>
  ...
  public renderValue: React.FC<FormFieldValueRendererProps<FormFieldBoolean>> =
    (props) => (
      <Text>
        {props.value === undefined
          ? "[Not set]"
