  ...
  useInput(async (input, key) => {
    if (engaged) {
      ...
      if (key.return) {
        ...
        const generatedDestinationIds = generateIds(
          parseInt(destinations),
          "dst"
        );
        const generatedConnectionIds = generateIds(
          parseInt(connections),
          "conn"
        );
        const generatedRequestIds = generateIds(
          parseInt(numberOfRequests),
          "req"
        );
        ...
        await db.transaction(async (tx) => {
          for (let i = 0; i < generatedConnectionIds.length; i++) {
            await tx.insert(connections).values({
              ...
              destinationId:
                destinationObjects[i % destinationObjects.length].insertId,
            });
          }
        });
        ...
        for (let i = 0; i < parseInt(numberOfRequests); i++) {
          ...
          promises.push(
            tb.responses.publish({
              ...
              source_id: generatedSourceIds[i % generatedSourceIds.length],
              destination_id:
                generatedDestinationIds[i % generatedDestinationIds.length],
              request_id: generatedRequestIds[i],
              body: content.body,
              headers: content.headers,
              status,
