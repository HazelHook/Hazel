 ...
 * @public
 */
export class HazelMiddleware<TOpts extends MiddlewareOptions> {
	/**
...
> &
	Omit<
		Partial<{
			[K2 in keyof TRecord]: (...args: Parameters<TRecord[K2]>) => Promise<Parameters<TRecord[K2]>[0]>
		}>,
		keyof FnsWithSameInputAsOutput<TRecord>
	>
...
>(
	...
	for (const k of Object.keys(ret)) {
		...
		ret[key] = cacheFn(ret[key] as (...args: unknown[]) => unknown) as unknown as TRet[keyof TRet]
	}
