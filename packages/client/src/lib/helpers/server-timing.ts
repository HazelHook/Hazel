...
export class ServerTiming {
	...
	public start(name: string, description?: string): () => void {
		...
		const index = (this.#timings[name] as Timing).timers.push({ start: Date.now() }) - 1

		return (): void => {
			const target = this.#timings[name]
			if (!target) {
				console.warn(`Timing "${name}" does not exist`)
			...
			if (!timer) {
				console.warn(`Timer ${index} for timing "${name}" does not exist`)
	...
	public getHeader(): string {
		const entries = Object.entries(this.#timings).reduce<string[]>((acc, [name, { description, timers }]) => {
			/**
			...
			const entry = [name, description ? `desc="${description}"` : "", dur ? `dur=${dur}` : ""]
				.filter(Boolean)
				.join(";")
